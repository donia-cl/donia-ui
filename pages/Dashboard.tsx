          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 max-w-2xl">
                ...
                <div className="relative z-10">
                   ...
                   <div className="space-y-6">
                      <div className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100">
                         ...
                         <div className={`${(isEmailUnverified && !isGoogleUser) ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'} px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2`}>
                            {(isEmailUnverified && !isGoogleUser) ? 'Pendiente' : (isGoogleUser ? 'Google' : 'Verificado')}
                            {isGoogleUser && <CheckCircle2 size={10} />}
                         </div>
                      </div>

                      {isEmailUnverified && !isGoogleUser && (
                      ...
                      {!isGoogleUser ? (
                        ...
                        <div className="p-6 bg-violet-50 rounded-3xl border border-violet-100 flex items-start gap-4">
                           <div className="bg-white p-2.5 rounded-xl shadow-sm text-violet-600"><Info size={20} /></div>
                           <div>
                              <p className="text-xs font-bold text-violet-900 mb-1">Cuenta vinculada con Google</p>
                              <p className="text-[11px] text-violet-700/70 font-medium leading-relaxed">Tu seguridad es gestionada externamente. Para cambiar tu correo o contraseña, debes hacerlo desde la configuración de tu cuenta de Google.</p>
                           </div>
                        </div>
                      )}
             ...
             <div className="bg-rose-50 border border-rose-100 p-8 rounded-[40px] flex items-start gap-5">
                ...
                <div>
                   ...
                   <p className="text-rose-800/70 text-xs font-medium leading-relaxed mb-4">Si sospechas que alguien ha accedido a tu cuenta sin permiso, te recomendamos cambiar tu contraseña inmediatamente o revisar tus accesos en Google.</p>
