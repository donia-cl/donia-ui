...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (type === 'summary') {
      ...
      return res.status(200).json({ success: true, data: { totalRecaudado, disponibleRetiro, enProceso, totalRetirado } });
    } else if (type === 'withdrawals') {
      // Antes en api/withdrawals.ts
      const { data, error } = await supabase.from('withdrawals').select('id, monto, fecha, estado, campaign_id, campaigns(titulo)').eq('user_id', userId).order('fecha', { ascending: false });
      if (error) throw error;
      const mapped = (data || []).map((w: any) => ({
        id: w.id, monto: w.monto, fecha: w.fecha, estado: w.estado,
        campaignId: w.campaign_id, campaignTitle: w.campaigns?.titulo || 'Campaña eliminada'
      }));
