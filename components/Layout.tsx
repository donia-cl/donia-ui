...
import { Heart, Database, Cpu } from 'lucide-react';
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row justify-between items-center gap-8">
              ...
              <div className="flex flex-wrap justify-center gap-4">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold border ${
                  dbStatus === 'cloud' 
                  ? 'bg-emerald-50 border-emerald-100 text-emerald-700' 
                  : 'bg-amber-50 border-amber-100 text-amber-700'
                }`}>
                  <Database size={14} />
                  {dbStatus === 'cloud' ? 'Nube (Conectado)' : 'Local (Almacenamiento)'}
                </div>
                ...
                <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold border ${
                  ...
                  <Cpu size={14} />
                  {aiActive ? 'IA Activa' : 'IA Desactivada'}
                </div>
              </div>
            </div>
          </div>
